...
static void test_video_frame_during_marker_interleave_is_blocked() {
    ...
    for (int iter = 0; iter < N_INTERVALS; ++iter) {
        ...
        if (out[marker_idx]->type != 1 || out[marker_idx]->data.GetSize() != 24) {
            FAIL("item after BEGIN is not the 24-byte marker — interleave occurred");
            free_drained(out); done.store(true); frame_thread.join(); return;
        }
...
static void test_video_cold_start_marker_only_first_interval() {
    ...
    if (out[1]->type != 1 || out[1]->data.GetSize() != 24) { FAIL("cold-start item 1 != marker24"); free_drained(out); return; }
    free_drained(out);
    ...
    for (int k = 0; k < 10; ++k) {
        ...
        if (iter_out[2]->type != 1 || iter_out[2]->data.GetSize() != 24) { FAIL("iter: item 2 != marker24"); free_drained(iter_out); return; }
