...
status: in_progress
stopped_at: 15.1-07b STABILIZATION (build 261 VST3 — UAT pending). 07b landed (commits dbdaf98/edb2769/b43aeb6) but real-DAW UAT exposed three regressions, each fixed: (1) e631aef restored on_new_interval bcast_active state machine (executor port had dropped the legacy state-transition logic — was emitting boundary AND stop markers unconditionally every interval, never set lcm.bcast_active=true for normal channels); (2) 1633bfd guards NJClient::Run encoder loop against null m_netcon (Disconnect tears down m_netcon before per-channel state, drainBroadcastBlocks is a NEW writer to lc->m_bq from the run thread on every Run() tick — encoder loop then derefs null on next tick → DAW crash); (3) 3799e8a removes _reinit channel_idx renormalize that was silently overwriting channel_idx on the canonical Local_Channel during every Disconnect, stranding the audio-thread mirror entry (mirror is indexed BY channel_idx — pre-Connect syncInstatalkBroadcast() added ch=4, then Connect→Disconnect→_reinit renamed to ch=0, mirror[4] kept .active=true and mirror[0] stayed .active=false, so handleStatusChange's SetLocalChannelInfo(0,"Ch1") found existing canonical and published InfoUpdate, never AddedUpdate). Build 261 awaiting UAT covering: local Ch1-4 VU moves with DAW audio, peers hear DAW audio, Disconnect doesn't crash, Disconnect→reconnect path works. Full diagnostic at .planning/phases/15.1-rt-safety-hardening/.continue-here.md and .planning/debug/build253-vu-and-disconnect.md.
...
Phase: 15.1 (rt-safety-hardening) — IN PROGRESS (Wave 1 + 15.1-05 + 15.1-06 complete; 15.1-07b stabilization in flight, build 261 UAT pending)
Plan: 7 of 11 confirmed done; 15.1-07b is awaiting UAT; 15.1-07a/07c, 08, 09, 10 remaining (DO NOT START 07a until 07b UAT passes)
...
Progress: [######....] 55% (v1.2 milestone — 6 of 11 sub-plans complete; advancing to 15.1-07a)
