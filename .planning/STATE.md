...
status: in_progress
stopped_at: 15.1-07b STABILIZED — UAT approved 2026-04-27 (build 261). Three regression fixes verified end-to-end against live DAW: (1) e631aef restored on_new_interval bcast_active state machine; (2) 1633bfd guards NJClient::Run encoder loop against null m_netcon on Disconnect; (3) 3799e8a removes _reinit channel_idx renormalize that was stranding the audio-thread mirror by silently overwriting channel_idx on the canonical Local_Channel without publishing a mirror update (Bug A — mirror is indexed BY channel_idx, so pre-Connect syncInstatalkBroadcast()'s SetLocalChannelInfo(4,…) followed by Connect→Disconnect→_reinit renaming to channel_idx=0 left mirror[4].active=true and mirror[0].active=false; subsequent SetLocalChannelInfo(0,"Ch1") hit was_add=false and published InfoUpdate, never AddedUpdate). Bug A was a "shadow representation drift" — same shape recorded in feedback_legacy_invariant_audit.md. Throwaway debug instrumentation from commits 1dd8db3/c3420c5/953d006 to be removed before 07a dispatch. Next: 15.1-07a (RemoteUserMirror, CR-01) — same architectural pattern as 15.1-06 for Local_Channel, but on the RemoteUser side; needs extra care around RemoteUser_Channel.channel_index mirror invariant. Architecture-mirror audit owed before 15.1-10. Full diagnostic preserved at .planning/phases/15.1-rt-safety-hardening/.continue-here.md and .planning/debug/build253-vu-and-disconnect.md.
...
Phase: 15.1 (rt-safety-hardening) — IN PROGRESS (Wave 1 + 15.1-05 + 15.1-06 + 15.1-07b complete and UAT-approved; advancing to 15.1-07a)
Plan: 8 of 11 done (15.1-07b UAT approved 2026-04-27 build 261 covering local-Ch1-4 VU, peer-broadcast, Disconnect-no-crash, Disconnect→reconnect path); 15.1-07a/07c, 08, 09, 10 remaining
