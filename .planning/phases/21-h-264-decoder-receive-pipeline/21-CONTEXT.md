...
3. **Per-peer libavcodec H.264 decoder** running on its own `juce::Thread`. Pure consumer of a `SpscRing<NalChunk, 32>` populated by the run thread (AVCC parsing happens on the run thread; decoder is codec-API-only). YUV420P → BGRA via `libswscale sws_scale` into the peer's distributor sink.
...
- **D-09: One libavcodec decoder thread per peer (`juce::Thread`).** Each `VideoRecvState` owns its own decoder thread + AVCodecContext + SwsContext + per-peer `SpscRing<NalChunk, 32>` populated by the run thread. Isolation: one peer's slow / corrupted decode doesn't stall any other peer or the run thread. Symmetric to Phase 20's encoder threading (one thread per encoder instance). At 8 peers = 8 decoder threads, fine on modern desktop. Decoder thread loops: pop NalChunk → `avcodec_send_packet` → `avcodec_receive_frame` loop → `sws_scale` into `image_back` → swap front/back → bump generation → triggerAsyncUpdate.
...
- **D-12: AVCC parsing on run thread; decoder thread is pure libavcodec consumer.** When a slot's bytes are dispatched to the decoder (via the audio-thread SWAP, after GUID-pairing places it in `playing`), the run thread walks the slot's bytes: 24-byte marker → SPS/PPS chunk (`[4B BE outer_len][2B BE sps_len][SPS_no_start_code][2B BE pps_len][PPS_no_start_code]`) → per-frame AVCC chunks (`[4B BE nal_len][NAL]`). For each parsed item, push a structured `NalChunk` to the per-peer decoder SPSC. Decoder thread is then a pure libavcodec consumer: each pop drives one `avcodec_send_packet()`. Clean separation; run thread handles wire-format details, decoder handles codec-API details. AVCC parsing is cheap (~10 µs per HD interval) — fine on the run thread.

- **D-13: SPS/PPS fed as Annex-B packets via same code path as frame NALs (no avcC extradata).** Run thread parses the SPS/PPS chunk inner into `[SPS_NAL]` + `[PPS_NAL]`. Wraps each with Annex-B start code (`00 00 00 01`) and pushes as a `NalChunk{kind: ParamSet}` on the decoder SPSC. Decoder feeds them via `avcodec_send_packet` — libavcodec recognizes NAL `unit_type 7` (SPS) and `8` (PPS) and updates internal state. No `AVCodecContext::extradata` setup; no `AV_CODEC_FLAG_GLOBAL_HEADER`. Same code path as frame NALs; handles mid-stream SPS/PPS updates (peer preset change) without special-casing.
...
- **D-16: Phase 21 audio-thread audit-allowlist envelope (parallel to Phase 20 D-09).** Plan 21-XX writes the following carve-out entries to `.claude/agents/realtime-audio-reviewer.md`:
  ...
  - `SpscRing<NalChunk, 32>::try_push` on `vs->decoderInputQ` (already lock-free, RT-safe).
...
- **`SpscRing<NalChunk, 32>` depth** — 32 NALs holds ~2 intervals at worst; planner picks if more headroom is needed.
- **`NalChunk` storage** — small inline buffer + heap fallback for large NALs, or always-heap. Planner picks based on size distribution analysis.
...
- **`SpscRing<T, N>`** (`src/threading/spsc_payloads.h`) — Phase 15.1-04 substrate; Phase 21 reuses for `VideoRecvState::decoderInputQ` (run thread → decoder thread NAL handoff). Already RT-safe, MAX_BLOCK_SAMPLES contract documented at source, two-layer enforcement at allocations.
...
- **Pure libavcodec consumer thread loop body:** kept tight and obvious — pop, send_packet, drain receive_frame loop, sws_scale, swap, generation+, async update. No business logic in the decoder thread; all wire-format / pipeline-state work stays on the run thread or audio thread.
