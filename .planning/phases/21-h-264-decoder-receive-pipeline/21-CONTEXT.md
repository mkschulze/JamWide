...
- **D-16: Phase 21 audio-thread audit-allowlist envelope (parallel to Phase 20 D-09).** Plan 21-XX writes the following carve-out entries to `.claude/agents/realtime-audio-reviewer.md`:
  ...
  - Slot-view push onto the per-peer decoder SPSC (either `SpscRing<VideoRecvBufferView, 4>::try_push` or a single memcpy into a pre-allocated decoder-owned buffer — Plan 21-02 picks the cleanest; both are already lock-free / RT-safe and replace the previously envisioned `SpscRing<NalChunk, 32>::try_push` allowlist entry — the parsed NalChunks are now produced by the decoder thread per D-12 revised and never cross the audio-thread boundary).
...
- **Audio→decoder handoff SPSC choice** — slot-view pointer push (`SpscRing<VideoRecvBufferView, 4>`) versus single memcpy into a decoder-owned bounded buffer. Per D-12 revised, Plan 21-02 picks the cleanest; both are RT-safe.
