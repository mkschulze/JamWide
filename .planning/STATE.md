...
Phase: 15.1 (rt-safety-hardening) — IN PROGRESS (Wave 1 + 05 + 06 + 07b + 07c + 09 done; 07a impl complete UAT-deferred; advancing to 08)
Plan: 10 of 11 done (09 CR-08/H-04/CR-11-partial closed via file-reader inversion + run-thread refill — every audio-thread-visible DecodeState now has decode_fp==nullptr in shipped state; build 282 VST3 + 280 Standalone + TSan-Standalone all green; 8/8 tests Release + TSan zero TSan reports). Deviation noted: plan's literal mixInChannel grep was obsolete because 07a had already eliminated the audio-thread caller — inversion wired at MESSAGE_SERVER_DOWNLOAD_INTERVAL_BEGIN (njclient.cpp:1968) and RemoteDownload::startPlaying:4970 instead. m_arm_request_q SPSC + drainArmRequests dormant for now (no audio-thread emitter today; infra preserved for future sessionmode re-enable). 07a UAT deferred to phase-end 15.1-10. Remaining: 08 (prealloc, depends on 09), 10 (verification + bundled UAT).
