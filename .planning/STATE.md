...
status: in_progress
stopped_at: 15.1-07a IMPL COMPLETE — UAT pending (build 269 Standalone TSan + build 268 VST3, auto-installed). Tasks 1-4 of 5 landed in commits bc3d9e4 (tests, prior session) + 582b906 (impl) + ea4b0b1 (SUMMARY). m_users_cs.Enter/Leave removed from all 4 audio-path sites (AudioProc, process_samples remote-user mix loop, on_new_interval remote-user advance, mixInChannel) — closes AUDIT CR-01. mixInChannel signature changed from (RemoteUser*, int chanidx, …) to (int slot, int chanidx, …); audio thread no longer dereferences any run-thread-owned RemoteUser/RemoteUser_Channel object — Codex HIGH-2 closed (no user_ptr/RemoteUser*/void* escape hatch in mirror). m_remoteuser_mirror[MAX_PEERS=64] keyed by STABLE SLOT allocator (NOT m_remoteusers list index) — mitigates the Bug-A-shape risk surfaced by 15.1-MIRROR-AUDIT.md (m_remoteusers.Delete shifts list indices). m_remoteuser_deferred_delete_q + m_audio_drain_generation gate operational; peer remove (auth-side per-peer) and bulk Disconnect both use publish-wait-defer (200ms wait, leak on timeout to preserve RT-safety) — Codex HIGH-3 closed. All 4 audit-cited m_remoteusers mutation sites publish RemoteUserUpdate variants. PeerNextDsUpdate publishing wired at MESSAGE_SERVER_DOWNLOAD_INTERVAL_BEGIN and RemoteDownload::startPlaying. drainRemoteUserDeferredDelete wired in juce/NinjamRunThread.cpp (in-loop + graceful shutdown). spsc_payloads.h UNTOUCHED (Codex M-9 preserved). Tests: 6/6 Release + 6/6 TSan (test_remote_user_mirror 8/8 + 5 regressions in each), zero ThreadSanitizer reports. Builds: VST3 (268), Standalone (269), TSan-Standalone all green. 4 deviations documented in 15.1-07a-SUMMARY.md (3 Rule 3 blocking — DecodeState namespace bridge / run_thread_next_ds_idx shadow / Disconnect path coverage; 1 plan-faithful expansion — defer-delete on PeerChannelMaskUpdate present→not-present transition). Awaiting UAT covering 10-min populated-server peer-churn TSan stress (5+ disconnect/reconnect cycles, mute/solo/vol/pan toggling, listen for remote audio glitches). Plan is autonomous: false.
...
Phase: 15.1 (rt-safety-hardening) — IN PROGRESS (all 10 impl plans landed; 15.1-10 mechanical verification GREEN; awaiting bundled human-verify UAT covering 07a/07c/09/08 + phase-close ceremony per CONTEXT D-08)
Plan: 15.1-10 mechanical signals 1a/1b/2/4/5 GREEN (build 287 TSan Standalone). Signal 1a — 9/9 RT-safety tests pass under TSan zero reports. Signal 1b — NEW test_peer_churn_simulation 1/1 PASS Release + TSan (3 threads × 1000 cycles, drop-counter zero gate enforced in-test). Signal 2 — closing audit CRITICAL=0 (12→0 across phase), all 21 pre-fix findings dispositioned, Codex HIGH-1/HIGH-2/HIGH-3 special-checks PASS. Signal 4 — m_users_cs/m_locchan_cs/m_misc_cs absent from all audio-path function ranges (m_misc_cs zero matches anywhere). Signal 5 — fopen/fread/writeLog/writeUserChanLog absent from audio-path; line 478 fread VERIFIED-UNREACHABLE per HIGH-1 invariant. spsc_payloads.h verification: ONE commit ever touches this file (8f47965 from 15.1-04 inception) — Codex M-9 Wave-0 finality preserved end-to-end. Architecture-mirror dead-but-live finding (njclient.cpp:1648 lobby renormalize) flagged in deferred-items.md per option (b) — VERIFIED-UNREACHABLE in modern JamWide. Awaiting HUMAN signals: 1c (manual standalone UAT under TSan, 30+ min populated server, bundled-UAT scope explicit per feedback_uat_scope_redflags.md), 3 (audible glitches + CPU history pattern check), 6 (lldb attach + read 6 production drop-counter accessors at session-end, all must == 0), 7 (Instruments Time Profiler 60s under broadcast load, BlockRecord memcpy CPU < 5%). Build for UAT: build-tsan/JamWideJuce_artefacts/Debug/Standalone/JamWide.app (build 287). On UAT approval: phase 15.1 closes, v1.2 milestone closure proceeds, PR #14 (VDO sync) hold released.
